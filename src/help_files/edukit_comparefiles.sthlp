...
{phang}
{cmd:edukit_comparefiles}, {opt localfile(filepath)} {opt sharedfile(filepath)} [{opt idvars(varlist)} {opt idsfromchar(string)} {opt compareboth} {opt comparelocal} {opt compareshared} {opt comparevars(varlist)} {opt wigglevars(varlist)} {opt wiggleroom(numlist)} {opt mdreport(filepath)} {opt varlistlenmax(numlist)} {opt mdevensame}]
{p_end}
